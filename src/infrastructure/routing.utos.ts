...
export type RoutesReturn<R extends RoutesType> = {
  [K in keyof R]: (params?: {
    ...
    //component?: { [PK in keyof R[K]["component"]]?: React.ReactNode };
  }) => string;
};
 ...
 */
export const makeRouteMap = <R extends RoutesType>(routes: R, options?: MakeRouteMapOptions): RoutesReturn<R> => {
    ...
    const func: RoutesReturn<R>[typeof key] = (routeInfo?: {
      ...
      if (routeInfo?.params) {
        Object.entries(routeInfo.params).forEach(([paramName, value]) => {
          newPath = newPath.replace(options?.paramMatcher?.(paramName) || new RegExp(":" + paramName), String(value));
        });
      }
      if (!routeInfo?.search) {
        return newPath;
      } else {
        return `${newPath}?${new URLSearchParams(routeInfo.search as any).toString()}`;
      }
    };
 ...
 */
export const makeNavigate = <R extends RoutesType>(routeMap: RoutesReturn<R>, goToRoute: (route: string) => void): UseNavigateReturn<R> => {
